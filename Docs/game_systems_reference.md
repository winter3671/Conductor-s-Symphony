...
8. **드럼 오라는 M_rhythm 미적용**: 판정 성공과 무관한 상시 효과라 리듬 정확도 배율은 제외하고
   시포르찬도(M_stat)만 반영.
9. **레가토(Legato) 패시브 / 악기 Lv4 "Multi+1" 스탯이 여전히 비활성 상태**: "투사체 수 +1/+2"와 각
   악기 Lv4의 `extraProjectiles`는 과거 범용 폴백 투사체 로직에서만 소비되던 값입니다. 10종 전체가
   전용 디스패처로 처리되면서 이 폴백 로직에 도달하지 않게 됐고(신규 악기 추가 대비 안전장치로 코드에는
   의도적으로 남아있음 — 6절 "완료 항목" 참고), UI에는 "Multi +N"이 표시되지만 실제 게임플레이엔
   영향이 없습니다. **8종 패시브 중 유일하게 아직 해결되지 않은 항목**입니다(4절 참고).
...
**크레센도(범위)·알레그로(쿨타임 감축)·페르마타(지속시간 증가) 3종은 계산 함수(`PassiveStatManager`)는
정확했지만 그 값을 실제로 소비하는 코드가 프로젝트 전체에 단 한 곳도 없어 완전히 죽어있었습니다.**
드럼의 범위 표시 버그를 조사하다가 크레센도가 죽어있음을 처음 발견했고, 이후 나머지 7종 패시브 전부를
"실제 소비 코드 유무" 기준으로 감사해 알레그로·페르마타도 같은 상태임을 추가로 확인했습니다. 지금은
셋 다 10종 악기에 연동을 마쳤고, **레가토만 유일하게 미해결로 남아있습니다**(3절 9항).
...
**레가토 패시브·Lv4 "Multi+1" 스탯이 비활성 상태** — 3절 9항 참고. "추가 투사체"가 악기별로 구체적으로
무엇을 의미해야 하는지(피아노는 발사 수 추가? 벨은 방향 추가?)를 정의하거나, 레가토를 다른 효과로
교체할지 결정이 필요합니다.

**`GetTotalExtraDamage()`가 판정 악기가 아니라 장착 4슬롯 전체를 합산** — 의도한 설계(팀 시너지)인지
버그인지 확정 필요. 영향은 작음(악기당 최대 +2, 4개 합쳐도 +8).

**데미지 반올림 규칙(banker's rounding)** — `Mathf.RoundToInt`는 `.5`를 가장 가까운 짝수로 내림합니다.
체감 손실이 느껴지면 `Mathf.CeilToInt`로 전환 검토.

**전역 콤보 공유** — 피아노/글록켄슈필 버스트가 "자기 악기 콤보"가 아니라 전역 콤보를 재사용합니다
(3절 2항). 유지할지, 악기별 개별 콤보 카운터를 도입할지 결정 필요.

### 7-2. 기획서 대비 구현이 달라진 부분 (설계 의도 확인 필요)

**2-1. 첼로 Lv4 "지속시간 +30%" → "릴리즈 후 잔류시간"으로 재해석** — 홀드 중에만 유지되는 구조라
"자동으로 지속시간이 늘어난다"는 개념이 성립하지 않아, "홀드를 뗀 뒤 1.3초 더 잔류"로 재해석했습니다.
다른 대안(홀드 유지 가능 시간 자체 연장)도 가능해 기획 의도 확인 필요.
...
**2-5. 벨의 8방향 발사가 "가장 가까운 적" 1명에게 항상 전부 명중** — 보스가 "가장 가까운 적"으로
선택되면 보스도 8~16발을 동시에 맞아, 벨 장착 시 보스전 난이도가 유독 쉬워질 수 있습니다. "벨 = 단일
타겟 집중 처형기" 컨셉이면 문제없지만, 다른 악기와 비슷한 수준을 의도했다면 발사 원점 변경/중복 판정
방지가 필요.
...
**4-1. "리듬 노트 종합 설계 가이드"와 실제 구현이 완전히 다른 스코프** — 가이드 문서는 1~16마디 전체를
마디마다 다르게 짠 완성형 노트 차트인데, 실제 구현(`InstrumentPatternDatabase.cs`)은 "레벨 1~5마다
32스텝 패턴 1개가 계속 반복"되는 구조입니다. 이 가이드가 (a) 지금 방식으로 대체된 예전 기획이라 더
이상 유효하지 않은지, (b) 나중에 "한 곡 완주" 모드나 보스 전용 패턴을 만들 때 쓸 별도 자료인지 확인
필요.

**4-2. (사소함) 벨 노트 패턴 코드 주석이 실제 패턴과 다름** — `InstrumentPatternDatabase.cs`의 벨
Lv4/Lv5 주석("8-accent"/"9-accent")이 실제 문자열(6개/7개)과 다릅니다. 게임 동작에는 영향 없음,
다음에 이 파일을 만질 때 주석만 정리하면 됩니다.
